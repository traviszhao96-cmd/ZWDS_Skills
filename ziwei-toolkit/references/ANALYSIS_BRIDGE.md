...
>
> **原则一**：结论先行，依据后置。用户看到的是"你会遇到什么问题"，不是"你某宫有什么星"。
>
> **原则二**：**不做不会错，硬做必出错。** 只有满足信号阈值的模块才输出详细分析。不够格的模块，输出一行正面/中性结论即可。安静比噪音更有价值。
>
> **原则一**：结论先行，依据后置。用户看到的是"你会遇到什么问题"，不是"你某宫有什么星"。
>
> **原则二**：**不做不会错，硬做必出错。** 只有满足信号阈值的模块才输出详细分析。不够格的模块，输出一行正面/中性结论即可。安静比噪音更有价值。
...
| 4 | 学业 | 父母 | 疾厄、福德 | 父母宫=师承/学历（非原生关系，区别于家庭模块）；疾厄=身体/认证（文昌文曲）；福德=思维品质/悟性 |
| 5 | 恋爱 | 子女 | 夫妻、迁移 | 子女=桃花/恋爱对象；迁移=外缘；左辅右弼桃花规则 |
| 6 | 交友 | 交友 | 迁移、兄弟 | 兄友线互参；交友冲夫妻>婚姻被外部破坏 |
| 7 | 婚姻 | 夫妻 | 迁移、福德、田宅、交友 | 迁移冲夫妻=忌冲忌最凶；福德=婚姻满意度；田宅=成家；交友三八为朋 |
| 8 | 子女 | 子女 | 田宅、夫妻 | 子女=孩子本身；田宅=家庭环境；夫妻=生育意愿 |
| 9 | 身心与生活 | 命宫 | 疾厄、福德、迁移 | 命宫=自我底色；疾厄=身体/性格暗面；福德=精神世界；迁移=外在表现 |
...
**每个模块必须在至少一个高信号上触发，才展开详细分析。否则只输出一行简评。**

| 模块 | 高信号（触发则展开） | 低信号（仅简评，不展开） |
|:--|:--|:--|
| 事业 | 官禄生年忌/权、官禄忌冲、迁移忌冲官禄、交友忌冲 | 七杀/破军等主星特性 |
| 财富 | 财帛生年忌、田宅飞忌、财帛自化忌、禄存被破、地劫/地空在财帛+忌 | 一般破军配地空 |
| 家庭 | 父母/兄弟宫忌冲、火星/铃星+忌、太阳太阴陷+忌 | 主星配置一般 |
| 学业 | 文昌化科/忌(父母/疾厄/命宫)、文曲科/忌(父母/福德)、天机权/科在父母/福德、父母生年忌/自化忌、文昌科@疾厄(身体认证型)、巨门忌@福德 | 一般天机/文昌无化 |
| 恋爱 | 子女忌、左右化科、迁移忌飞子女、子女自化忌 | 一般主星配置 |
| 交友 | 交友忌冲夫妻、交友飞忌入命、兄友线双向忌 | 交友空宫（不一定值得写） |
| 婚姻 | 夫妻忌、迁移忌**对冲**夫妻、夫妻自化忌、三八为朋破、左右化科在夫妻 | 夫妻地劫/自化权(无其他叠加) |
| 子女 | 子女忌、田宅忌冲子女、夫妻自化忌 | 一般主星配置 |
| 身心 | 命宫自化忌、疾厄忌、迁移忌冲命、身宫落忌、福德飞忌入命 | 一般贪狼/破军/天机等动星 |

**简评模板（无高信号时）**：
```
婚姻：武曲天府旺庙，配偶刚强稳重、靠得住。整体平顺。
```
> 简评不是"没东西写"。即使无风险信号，直接读宫给画像（星曜特质 → 人物画像）本身就是输出价值。

**展开模板（有高信号时）**：
```
婚姻：XXX。
- 风险：XXX（依据：夫妻宫生年忌 + 迁移忌冲）
- 提示：XXX（依据：三八为朋破）
```
...
输入（多来源碎片）:
  ...
  ├── 福德宫·飞忌入夫妻                  → "福德壬干飞武曲忌入夫妻（按有效飞宫规则，非生年四化宫位：无效）"
  ├── 交友宫·三八为朋                  → "交友无忌冲夫妻，外部破坏风险低"
  ├── 迁移宫·忌冲命宫                  → "廉贞忌冲命，命宫=夫妻的福德(转宫)，间接影响配偶精神"
  ├── 左辅右弼·桃花/Again             → "无化科桃花，无二度姻缘"
  └── 大限引动                        → "当前大限未过夫妻，暂无时间压力"

       ↓ 合成层（先判断信号阈值）

  高信号触发：福德飞忌入夫妻 → 无效飞宫，不构成高信号
            迁移忌冲夫妻 → 迁移对宫是命宫，不冲夫妻
            三八为朋 → 不破
            左右桃花 → 不触发
  → 阈值判断：无高信号 → 仅输出简评
...
输出:
  conclusion: "武曲天府旺庙，配偶稳重靠谱。无高信号。整体平顺。"
  severity: 'neutral'
```
